...
- [ ] Accueil produit posé : projet chez lui, profondeur, approches (extérieur / code / intérieur), accès oui/non
- [ ] Brief écrit : URL et/ou `code_path`, hors-scope, mode mappé — mots de passe hors chat (`accounts.local.md`)
...
- [ ] Après 11 : tickets `livrables/tickets/` avec prompt de correctif + compagnon d’implémentation
- [ ] Approche code : `SPECIALISTS/code-source` prévu ; sans `code_path` → `Non testé`
